...
from custom_demo.web import sandbox as WS
...
def _install_client(monkeypatch, client=None):
    ...
    monkeypatch.setattr(S, "SandboxClient", lambda **kw: client)
    return client
...
def test_skills_and_sandbox_compose(monkeypatch):
    ...
    default, routes = B._resolve_backends(_rt(customer="Eval Co", skills_repo="eval-skills"))
...
def test_sandbox_only_is_plain_backend(monkeypatch):
    _install_client(monkeypatch)
    default, routes = B._resolve_backends(_rt(customer="Eval Co"))  # no skills/agent repo
...
def test_available_seeds_data_stack_and_dataset_once(monkeypatch):
    ...
    B._resolve_backends(_rt(customer="Eval Co"))
...
def test_client_failure_falls_back_to_statebackend(monkeypatch):
    ...
    monkeypatch.setattr(S, "SandboxClient", _boom)
    default, routes = B._resolve_backends(_rt(customer="Eval Co"))
    assert isinstance(default, StateBackend)
...
def test_no_langsmith_key_skips_sandbox(monkeypatch):
    ...
    client = _FakeClient()
    monkeypatch.setattr(S, "SandboxClient", lambda **kw: client)
    default, _routes = B._resolve_backends(_rt(customer="Eval Co"))
    assert isinstance(default, StateBackend)
...
def test_prewarm_creates_and_seeds_then_runtime_reuses(monkeypatch):
    client = _install_client(monkeypatch)
    S.prewarm_sandbox(customer="Eval Co", seed=_SEED)
...
def test_prewarm_uses_agent_repo_precedence(monkeypatch):
    ...
    client = _install_client(monkeypatch)
    S.prewarm_sandbox(agent_repo="acme-agent", customer="Acme", seed=_SEED)
...
def test_legacy_context_hub_assistant_keeps_whole_repo(monkeypatch):
    client = _install_client(monkeypatch)
    _stub_ctxhub(monkeypatch)
    default, routes = B._resolve_backends(_rt(agent_repo="acme-agent", ls_workspace="ws"))
...
def test_sandbox_created_and_seeded_once_across_calls(monkeypatch):
    ...
    defaults = [B._resolve_backends(rt)[0] for _ in range(5)]
...
def _expire_cache(key: str = "Eval Co") -> None:
    ...
    S._SANDBOX_CACHE[key].validated_at -= S._SANDBOX_REVALIDATE_AFTER + 1
...
def test_fresh_cache_entry_is_trusted_without_asking_the_service(monkeypatch):
    ...
    first, _ = B._resolve_backends(_rt(customer="Eval Co"))
    ...
    again, _ = B._resolve_backends(_rt(customer="Eval Co"))
...
def test_a_stopped_vm_is_restarted_rather_than_replaced(monkeypatch):
    ...
    default, _ = B._resolve_backends(_rt(customer="Eval Co"))
...
def test_an_unavailable_vm_degrades_instead_of_serving_a_dead_handle(monkeypatch):
    ...
    assert _execute_offered(default, routes) is False
    assert "Eval Co" not in S._SANDBOX_CACHE
...
def test_attach_only_callers_never_provision_a_replacement(monkeypatch):
    ...
    assert S.attach_sandbox("Eval Co") is None
...
def test_sandbox_note_routes_file_requests_to_the_upload_button(monkeypatch):
    ...
    assert "pypdf" in note and "pypdf" in S._SEED_INSTALL
...
def test_seed_script_writes_the_use_cases_own_files():
    script = S.render_seed_script(_MEDICAL_SEED)
...
def test_seed_spec_is_capped():
    ...
    spec = json.loads(S.render_seed_script(many).split("<<'SPEC'\n", 1)[1].split("\nSPEC\n", 1)[0])
    assert len(spec["files"]) == S.SEED_MAX_FILES
    assert all(len(f["rows"]) <= S._SEED_MAX_ROWS for f in spec["files"])


def test_a_spec_with_nothing_usable_in_it_renders_no_script():
    assert S.render_seed_script([]) == ""
    assert S.render_seed_script([{"name": "", "kind": "csv"}]) == ""


def test_seeding_writes_the_assistants_own_spec():
    ran: list[str] = []
    backend = SimpleNamespace(execute=lambda script: ran.append(script))
    S._seed_data(cast("Any", backend), _MEDICAL_SEED)
...
def test_an_assistant_with_no_spec_fails_instead_of_borrowing_a_dataset():
    ...
    ran: list[str] = []
    backend = SimpleNamespace(execute=lambda script: ran.append(script))
    with pytest.raises(S.SeedSpecError, match="no `sandbox_seed` spec"):
        S._seed_data(cast("Any", backend), None)
...
def test_a_spec_the_model_mangled_says_so_rather_than_seeding_nothing():
    ...
    with pytest.raises(S.SeedSpecError, match="none of the 2 entries"):
        S._seed_data(
            cast("Any", backend),
...
def test_a_missing_spec_fails_the_turn_rather_than_degrading_to_no_vm(monkeypatch):
    ...
    """
    _install_client(monkeypatch)
    with pytest.raises(S.SeedSpecError):
        S.ensure_sandbox("mckesson-a1b2c3", seed=None)
...
def test_a_prewarm_that_cannot_seed_says_why(monkeypatch, capsys):
    ...
    _install_client(monkeypatch)
    S.prewarm_sandbox(sandbox_key="mckesson-a1b2c3", seed=None)
...
def test_a_lazily_created_vm_seeds_from_the_runs_context(monkeypatch):
    ...
    client = _install_client(monkeypatch)
    B._resolve_backends(_rt(customer="Mercy Health", sandbox_seed=_MEDICAL_SEED))
...
def test_two_assistants_for_one_customer_get_different_vms(monkeypatch):
    ...
    B._resolve_backends(first)
    B._resolve_backends(second)
...
def test_the_assistants_own_key_wins_over_the_customer_derived_ones():
    assert S.sandbox_key_from("mckesson-a1b2c3", "mckesson-agent", "McKesson") == "mckesson-a1b2c3"
...
def test_an_assistant_with_no_key_still_reaches_its_old_vm():
    ...
    assert S.sandbox_key_from("", "acme-agent", "Acme") == "acme-agent"
    assert S.sandbox_key_from(None, None, "Acme") == "Acme"
    assert S.sandbox_key_from(None, None, None) == "default"
...
def test_prewarm_and_the_runtime_agree_on_the_key(monkeypatch):
    ...
    S._SANDBOX_CACHE.clear()  # the runtime is a different process
    B._resolve_backends(
        _rt(
...
def test_an_agent_repo_that_cannot_back_a_filesystem_fails_the_turn(monkeypatch):
    ...
    with pytest.raises(B.BackendSourceError, match="acme-agent") as exc:
        B._resolve_backends(_rt(agent_repo="acme-agent", ls_workspace="ws"))
...
def test_a_skills_repo_that_cannot_be_mounted_fails_the_turn(monkeypatch):
    ...
    _install_client(monkeypatch)
    _ctxhub_boom(monkeypatch)
    with pytest.raises(B.BackendSourceError, match="eval-skills") as exc:
        B._resolve_backends(_rt(customer="Eval Co", skills_repo="eval-skills"))
...
def test_a_shared_vm_says_which_key_it_fell_back_to(capsys):
    S._KEY_SOURCE_REPORTED.clear()
    assert S.sandbox_key_from(None, "acme-agent", "Acme") == "acme-agent"
...
def test_the_shared_vm_warning_is_logged_once_not_once_per_resolve(capsys):
    ...
    S._KEY_SOURCE_REPORTED.clear()
    for _ in range(5):
        S.sandbox_key_from(None, None, "Acme")
...
def test_an_assistant_with_its_own_key_says_nothing(capsys):
    S._KEY_SOURCE_REPORTED.clear()
    assert S.sandbox_key_from("acme-a1b2c3", "acme-agent", "Acme") == "acme-a1b2c3"
...
def test_prewarm_runtime_and_browser_share_the_assistant_resource(monkeypatch):
    client = _install_client(monkeypatch)
    identity = {"sandbox_key": "acme-123", "agent_repo": "acme-agent", "customer": "Acme"}
    S.prewarm_sandbox(**identity, seed=_SEED)
    S._SANDBOX_CACHE.clear()
    _stub_ctxhub(monkeypatch)
    runtime_backend, _ = B._resolve_backends(_rt(**identity, skills_repo="acme-skills"))
    request = SimpleNamespace(query_params=identity)
    browser_backend, error = asyncio.run(WS._resolve_backend(request))
    assert error is None
    assert browser_backend is runtime_backend
    assert client.created == ["da-acme-123"]
    assert len(client.existing[0].runs) == 1


def test_browser_cannot_create_or_seed_an_absent_assistant_resource(monkeypatch):
    client = _install_client(monkeypatch)
    backend, error = asyncio.run(
        WS._resolve_backend(SimpleNamespace(query_params={"sandbox_key": "absent"}))
    )
    assert backend is None
    assert error.status_code == 503
    assert client.created == []
    assert S._SANDBOX_CACHE == {}


def test_browser_can_restart_a_stopped_resource_without_reseeding(monkeypatch):
    client = _install_client(monkeypatch)
    client.existing.append(_FakeSandbox("da-acme-123", status="stopped"))
    backend, error = asyncio.run(
        WS._resolve_backend(SimpleNamespace(query_params={"sandbox_key": "acme-123"}))
    )
    assert error is None
    assert isinstance(backend, LangSmithSandbox)
    assert client.started == ["da-acme-123"]
    assert client.created == []
    assert client.existing[0].runs == []


def test_cache_entry_keeps_backend_and_validation_time_together(monkeypatch):
    _install_client(monkeypatch)
    clock = SimpleNamespace(monotonic=lambda: 1000.0)
    monkeypatch.setattr(S, "time", clock)
    backend = S.ensure_sandbox("acme-123", seed=_SEED)
    entry = S._SANDBOX_CACHE["acme-123"]
    assert entry.backend is backend
    assert entry.validated_at == 1000.0
    clock.monotonic = lambda: 1000.0 + S._SANDBOX_REVALIDATE_AFTER + 1
    assert S.attach_sandbox("acme-123") is backend
    assert entry.validated_at == clock.monotonic()


def test_sandbox_hosting_scope_is_independent_of_context_hub_workspace(monkeypatch):
    client = _install_client(monkeypatch)
    seen = []
    monkeypatch.setenv("LANGSMITH_API_KEY", "deployment-key")
    monkeypatch.setenv("LANGSMITH_WORKSPACE_ID", "hosting-workspace")
    monkeypatch.setattr(S, "SandboxClient", lambda **kw: seen.append(kw) or client)
    _stub_ctxhub(monkeypatch)
    B._resolve_backends(
        _rt(sandbox_key="acme-123", skills_repo="acme-skills", ls_workspace="customer-workspace")
    )
    assert seen == [{"api_key": "deployment-key", "headers": {"X-Tenant-Id": "hosting-workspace"}}]


def test_one_dynamic_adapter_resolves_each_assistants_current_run(monkeypatch):
    client = _install_client(monkeypatch)
    adapter = B.DynamicBackend()
    monkeypatch.setattr(B, "get_runtime", lambda: _rt(sandbox_key="acme-first"))
    first = adapter.default
    monkeypatch.setattr(B, "get_runtime", lambda: _rt(sandbox_key="acme-second"))
    second = adapter.default
    assert first is not second
    assert client.created == ["da-acme-first", "da-acme-second"]
    monkeypatch.setenv("SANDBOX_ENABLED", "0")
